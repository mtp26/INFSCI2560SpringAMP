<?php
  ...
  if($keyword) {
    $query = "select * from ($query) q where q.description like '%$keyword%' or q.title like '%$keyword%' or q.studyId in (select studyId from KeywordMatch, Keyword where KeywordMatch.keywordId = Keywords.keywordId and keyword = '$keyword')";
  }
  if($id) {
